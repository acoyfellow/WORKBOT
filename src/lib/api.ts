...
export async function getContacts(locationId: string, query = '', limit = 20) {
  ...
  const result = await api<{ data?: { contacts: Contact[]; meta?: { total?: number } }; contacts?: Contact[] }>(`/contacts?${params}`);
...
export async function getConversations(locationId: string, limit = 20) {
  const params = new URLSearchParams({ locationId, limit: String(limit) });
  const result = await api<{ data?: { conversations: Conversation[] }; conversations?: Conversation[] }>(`/conversations?${params}`);
  return { conversations: result.data?.conversations || result.conversations || [] };
}

export async function getPipelines(locationId: string) {
  const result = await api<{ data?: { pipelines: Pipeline[] }; pipelines?: Pipeline[] }>(`/pipelines?locationId=${locationId}`);
  return { pipelines: result.data?.pipelines || result.pipelines || [] };
}

export async function getWorkflows(locationId: string) {
  const result = await api<{ data?: { workflows: Workflow[] }; workflows?: Workflow[] }>(`/workflows?locationId=${locationId}`);
...
export async function getActivities(locationId: string, options?: {
  ...
  const result = await api<{ data?: { activities: Activity[] }; activities?: Activity[]; stats?: Record<string, number> }>(`/activities?${params}`);
...
export async function getStats(locationId: string) {
  const [contacts, conversations, opportunities] = await Promise.all([
    api<{ total?: number; contacts?: any[] }>(`/contacts?locationId=${locationId}&limit=1`),
    api<{ total?: number; conversations?: any[] }>(`/conversations?locationId=${locationId}&limit=1`),
    api<{ total?: number; opportunities?: any[] }>(`/opportunities?locationId=${locationId}&limit=1`),
  ]);
  return {
...
export async function getTokenStatus(): Promise<TokenStatus> {
  const res = await api<{ token: TokenStatus }>('/status');
  return res.token;
}

export async function refreshToken(): Promise<{ status: string; message: string }> {
  const res = await fetch(`${API_BASE}/token/refresh-async`, { method: 'POST' });
  return res.json();
}
